...
class LinkHero
{
    ...
    function get_sitemap_url()
    {
        ...
        $file_headers = @get_headers($sitemap);
        if ($file_headers && strpos($file_headers[0], '404') == false) {
            if(strpos($file_headers[0], '301') !== false){
                foreach($file_headers as $id => $header){
    ...
    function lh_request_scan()
    {
        ...
        if ($sitemap_url !== false) {
            ...
            $res = wp_remote_post(
                $this->linkhero_api . '/wp301/add/check_urls',
                array(
                    'sslverify' => false,
    ...
    function lh_get_results()
    {
        ...
        file_put_contents($results_zip, fopen($this->linkhero_api . '/wp301/get/check_urls?' . http_build_query($request), 'r'));
