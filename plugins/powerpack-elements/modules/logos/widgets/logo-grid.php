	...
	protected function register_content_logo_grid_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'             => [
					'(desktop){{WRAPPER}} .pp-elementor-grid .pp-grid-item-wrap' => 'width: calc( ( 100% - (({{columns.SIZE}} - 1) * {{SIZE}}{{UNIT}}) ) / {{columns.SIZE}} ); margin-right: {{SIZE}}{{UNIT}}; margin-bottom: {{SIZE}}{{UNIT}};',
					'(tablet){{WRAPPER}} .pp-elementor-grid .pp-grid-item-wrap'  => 'width: calc( ( 100% - (({{columns_tablet.SIZE}} - 1) * {{SIZE}}{{UNIT}}) ) / {{columns_tablet.SIZE}} ); margin-right: {{SIZE}}{{UNIT}}; margin-bottom: {{SIZE}}{{UNIT}};',
					'(mobile){{WRAPPER}} .pp-elementor-grid .pp-grid-item-wrap'  => 'width: calc( ( 100% - (({{columns_mobile.SIZE}} - 1) * {{SIZE}}{{UNIT}}) ) / {{columns_mobile.SIZE}} ); margin-right: {{SIZE}}{{UNIT}}; margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		<div {{{ view.getRenderAttributeString( 'logo-grid' ) }}}>
			<# _.each( settings.pp_logos, function( item ) { #>
				<# if ( item.logo_image.url != '' ) { #>
					...
					#>
					<div class="pp-grid-item-wrap elementor-repeater-item-{{ item._id }} {{ item_wrap_custom_style_class }}">
