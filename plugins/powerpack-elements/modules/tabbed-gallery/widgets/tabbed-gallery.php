...
class Tabbed_Gallery extends Powerpack_Widget {
	...
	protected function register_style_tabs_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units'            => '',
				'selectors'             => [
					'{{WRAPPER}} .pp-gallery-filters-wrap' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render_filters() {
		...
		?>
		<div class="pp-gallery-filters-wrap">
	...
	protected function render_gallery_items() {
		...
		foreach ( $photos as $photo ) {
			...
			<div class="pp-tabbed-carousel-slide <?php echo esc_attr( $final_tab_label ); ?>" data-item-id="<?php echo esc_attr( $photo->id ); ?>">
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( $pp_grid_item_key ) ); ?>>
		...
		<div class="pp-gallery-image-icon-wrap">
			<span class="pp-gallery-image-icon pp-icon">
				...
				if ( $is_new || $migrated ) {
					...
					?><i <?php echo wp_kses_post( $this->get_render_attribute_string( 'link-icon' ) ); ?>></i><?php
				}
				?>
			</span>
		</div>
