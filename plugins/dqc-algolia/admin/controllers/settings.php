...
class Settings {
	...
	public static function adminMenu(){
		...
		add_menu_page(__( 'Algolia Settings', 'mvnAlg' ), __( 'Algolia', 'mvnAlg' ), 'manage_options', Registry::instance()->getPluginShortName().'_general_settings', 'MavenAlgolia\Admin\Controllers\Settings::showForm',$iconUrl );
	}
...
//			}
	...
	public function ajaxValidateIndex() {
		...
		$appId = !empty( $_POST['appId'] ) ? sanitize_text_field( $_POST['appId'] ) : '';
		$apiKey = !empty( $_POST['apiKey'] ) ? sanitize_text_field( $_POST['apiKey'] ) : '';
		if ( strpos($_SERVER['HTTP_HOST'], PRODSITE) !== false ) {
			$indexName = PRODINDEX;
        } else if ( strpos($_SERVER['HTTP_HOST'], STAGSITE) !== false ) {
			$indexName = STAGINDEX;
        } else {
			$indexName = DEVINDEX;
        }
