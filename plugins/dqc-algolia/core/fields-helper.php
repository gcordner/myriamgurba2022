...
//				$value = utf8_encode($value);
        ...
        foreach($post_types as $key => $post_type) {
            if ( $post_type != "" ) {
                ...
                foreach($taxonomy_objects as $slug => $taxonomy) {
                	if ( $slug != "post_format" && $slug != "author" ) {
	                	if ( $slug == "post_tag" ) {
	                		$taxonomies[$slug] = array( 'algoliaName' => $slug, 'isTag' => TRUE, 'forFaceting' => TRUE );
	                	} else {
	                		$taxonomies[$slug] = array( 'algoliaName' => $slug, 'isTag' => FALSE, 'forFaceting' => TRUE );
	                	}
	                }
                }
                $defaultPostsTypes[$post_type_object->name] = array( 
	...
	public static function getDomainName ( ) {
		...
		$domainName = array_shift( $domainArray );
