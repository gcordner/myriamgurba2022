<?php
    ...
    */
    ...
    function swift_slider( $atts, $content = null ) {
        ...
        $slide_order_by = ( $random == 1 || $random == '1' || $random == 'yes' ) ? 'rand' : 'post_date';
        $ss_args             = array(
        ...
        <?php if ( $swift_slider_slides->have_posts() ) {
            ...
            if ( ($type == "slider" || $type == "half") && ($slide_count > 1 || $slide_count == -1) && ( $nav == "1" || $nav == "yes" || $nav == "true" ) ) {
            ...
            if ( $continue == "true" || $continue == 1 ) {
                ...
                $swift_slider_output .= '<a href="' . apply_filters( 'swift_slider_continue_href', '#' ) . '" data-href="#" class="swift-slider-continue">'.$ss_continue_icon.'</a>';
            }
    ...
    if ( !function_exists( 'swift_slider_slide') ) {
        function swift_slider_slide( $postID, $slide_ID = "" ) {
            ...
            // Slide variables
            $swift_slider_slide = $bg_image = $bg_mp4 = $bg_webm = $bg_ogg = $bg_image_url = $background_valign = $bg_mp4_id = $bg_webm_id = $bg_ogg_id = $bg_mp4_url = $bg_webm_url = $bg_ogg_url = "";
            $bg_video_size = array();
            ...
            if ( function_exists('rwmb_meta') ) {
                ...
                $bg_image          = spb_get_post_meta( $postID, 'ss_background_image', true );
                $bg_mp4            = spb_get_post_meta( $postID, 'ss_background_video_mp4', true );
                $bg_webm           = spb_get_post_meta( $postID, 'ss_background_video_webm', true );
                $bg_ogg            = spb_get_post_meta( $postID, 'ss_background_video_ogg', true );
            }
            ...
            if ( $bg_type == "video" ) {
                ...
                $swift_slider_slide .= '<video class="video" poster="' . $bg_image_url . '" ' . $video_loop . ' ' . $video_mute . ' preload="auto" data-width="' . $bg_video_size['width'] . '" data-height="' . $bg_video_size['height'] . '" playsinline>';
