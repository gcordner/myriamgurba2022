<?php
    ...
    */
    ...
    ================================================== */
    if ( ! function_exists( 'sf_get_post_type' ) ) {
        function sf_get_post_type( $post_type ) {
            ...
            $post_array = array( __( "Blank", "swift-framework-admin" ) => 0 );
            foreach ($results as $post) {
                $post_array = array_merge( $post_array, array( __( $post->post_title, "swift-framework-admin" ) => $post->ID ) );
            }
    ...
    ================================================== */
    if ( ! function_exists( 'sf_get_taxonomiese' ) ) {
        function sf_get_taxonomies() {
            ...
            $results = get_taxonomies(array(), "objects");
            $taxonomy_array = array( __( "None", "swift-framework-admin" ) => "" );
            foreach ($results as $taxonomy_slug => $taxonomy) {
                if ( !in_array ( $taxonomy->name, $invalid_taxonomies ) ) {
                    ...
                    $taxonomy_array = array_merge( $taxonomy_array, array( __( $taxonomy->label, "swift-framework-admin" ) => $taxonomy_slug ) );
                }
            }
    ...
    ================================================== */
    if ( ! function_exists( 'spb_get_category_list' ) ) {
        function spb_get_category_list( $category_name, $filter = 0, $category_child = "", $frontend_display = false ) {
           ...
           $invalid_taxonomies = array(
            ...
            $category_list = array();
            if ( !in_array ( $category_name, $invalid_taxonomies ) ) {
                $args = array( 
                                'hide_empty' => false, 
                ...
                $get_category  = get_terms( $args );
                if ( isset($get_category) && count($get_category) > 0 ) {
                    foreach ( $get_category as $category ) {
                        if ( isset( $category->name ) ) {
                            $cat_parent = $category->parent;
                            $cat_name = $category->name;
                            while($cat_parent > 0) {
                                if ( $cat_parent != 0 && strpos($cat_name, "------") == false ) {
                                    $cat = get_term($cat_parent, $category_name);
                            ...
                            $category_list = $category_list + array( __( $cat_name, "swift-framework-admin" ) => $category->slug );
                        }
                    }
                }
                $category_list = array( __( "All", "swift-framework-admin" ) => "all" ) + $category_list;
            }
    ...
    ================================================== */
    if ( ! function_exists( 'sf_list_gallery' ) ) {
        function sf_list_gallery() {
            $gallery_list  = array();
            $gallery_query = new WP_Query( array( 'post_type' => 'gallery', 'posts_per_page' => - 1 ) );
            while ( $gallery_query->have_posts() ) : $gallery_query->the_post();
                $gallery_list[ get_the_title() ] = get_the_ID();
            endwhile;
            wp_reset_query();

            if ( empty( $gallery_list ) ) {
                $gallery_list[] = "No galleries found";
            }
    ...
    function spb_getImageBySize(
        ...
        if ( $thumbnail == '' && $attach_id ) {
            ...
            $img_srcset = wp_get_attachment_image_srcset( $attach_id, $thumb_size );
            ...
            if ( $p_img != "" ) {
                ...
                $thumbnail = '<img src="' . $p_img['url'] . '" width="' . $p_img['width'] . '" height="' . $p_img['height'] . '" alt="'.$alt.'" srcset="' . $img_srcset . '" />';
                //TODO: APPLY FILTER
            }
        }
    ...
    function spb_responsive_vis_list() {
	    ...
	    $array = array(
            ...
            __( 'Hidden on All Devices', 'swift-framework-plugin' )            => "hide"
		);
		return $array;
	...
	if ( ! function_exists( 'spb_carousel_arrows' ) ) {
		function spb_carousel_arrows() {
			...
			$carousel_arrows = apply_filters('spb_carousel_arrows_html', '<div class="carousel-arrows"><a href="#" class="carousel-prev" tabindex="-1" role="button"><span class="accessibility-text">Previous Slide</span><i class="ss-navigateleft"></i></a><a href="#" class="carousel-next" tabindex="-1" role="button"><span class="accessibility-text">Next Slide</span><i class="ss-navigateright"></i></a></div>');
	...
	if ( ! function_exists( 'spb_get_post_types' ) ) {
		function spb_get_post_types() {
            ...
            unset($post_types['guest-author']);
            unset($post_types['ct_template']);
            unset($post_types['gallery']);
            unset($post_types['elementor_library']);
            unset($post_types['amn_exact-metrics']);
            unset($post_types['acf-field']);
            unset($post_types['acf-field-group']);
            unset($post_types['wp_log']);
            ...
            unset($post_types['wphb_minify_group']);
            unset($post_types['wpcf7_contact_form']);
            unset($post_types['wpephpcompat_jobs']);
            unset($post_types['wp_quiz']);
            unset($post_types['mc4wp-form']);
            unset($post_types['wpcf7s']);
            unset($post_types['feedback']);
            ...
            foreach($post_types as $key => $post_type) {
                if ( $post_type == "" ) {
                    ...
                    unset($post_types[$key]);
                    $post_types = array_merge( array( __( $post_type_object->label, "swift-framework-admin" ) => $post_type ), $post_types );
                }
            }
                
            $post_types = array_merge( array( __( "All", "swift-framework-admin" ) => "" ), $post_types ); 
    ...
    ================================================== */
    if ( ! function_exists( 'spb_post_types_by_taxonomy' ) ) {
        function spb_post_types_by_taxonomy( $tax = 'category' ) {
            $out = array();
    ...
    /* GET AUTHORS
    ================================================== */
    if ( ! function_exists( 'spb_get_authors' ) ) {
        function spb_get_authors() {
            $authors = array();
            $users = get_users();
            ...
            foreach ($users as $author) {
                ...
                if ( count($posts) > 0 ) {
                    if ( $author->data->user_nicename != "" ) {
                        $authors = array_merge($authors, array(__( $author->data->display_name, "swift-framework-admin" ) => $author->data->user_nicename));
                    }
                }
            }

            if ( function_exists( 'get_coauthors' ) && function_exists("coauthors_wp_list_authors_object") ) {
                $args = array(
                    'echo'             => false,
                    'number'           => 20, // A sane limit to start to avoid breaking all the things
                );
                $post_author = coauthors_wp_list_authors_object($args);
                foreach($post_author as $author) {
                    $posts = get_posts(array("author" => $author->display_name));
                    if ( count($posts) > 0 ) {
                        if ( $author->user_nicename != "" ) {
                            $authors = array_merge($authors, array(__( $author->display_name, "swift-framework-admin" ) => $author->user_nicename));
                        }
                    }
                }
            ...
            $authors = array( __( "All", "swift-framework-admin" ) => "all" ) + $authors; 
