    ...
    abstract class SwiftPageBuilderShortcode extends SFPageBuilderAbstract {
        ...
        function getElementHolder( $width ) {  
            ...
            if ( isset($this->atts['title']) && $this->atts['title'] != "" ) {
                $el_name = $this->atts['title'];
            }

            if ( $this->settings["base"] == 'spb_team_member' && isset($this->atts['team_member']) && $this->atts['team_member'] != "" ) {
                $el_name = get_the_title( $this->atts['team_member'] );
            }

            if ( $this->settings["base"] == 'spb_counter' && isset($this->atts['subject']) && $this->atts['subject'] != "" ) {
                $el_name = $this->atts['subject'];
            }
            ...
            if( $this->settings["base"] == "spb_multilayer_parallax" ){
                $content_url = plugins_url() .'/socialdriver-framework/includes/page-builder/assets/img/blank_f7.gif';
            }else{
                $content_url = "";
            }
            ...
            if ( $this->settings["base"] == 'spb_accordion' || $this->settings["base"] == 'spb_tabs' || $this->settings["base"] == 'spb_tour'  || $this->settings["base"] == 'spb_gmaps' || $this->settings["base"] == 'spb_multilayer_parallax' ) {
                ...
                $output .= '<input name="el_name_editor" type="text" class="textfield validate" value="'. $el_name . ' " /><a class="el-name-save" href="#" title="Save"><span class="icon-save"></span></a></div>';   
                ...
                if ( $param['master'] == 'spb_icon_box_grid' ) {
                    ...
                    for ( $i = 1; $i <= $max_ocurrences; $i++) {
                        ...
                        if ( strpos($icon, '/') === false ) {
                            ...
                            $param_line .= '<li><div class="section_repeater"><div class="left_section"><div class="section_icon_image"><i class="svg-icon-picker-item" style="background-image: url(' . $icon . ');" data-path="' . $icon . '"></i></div><a href="#" class="section_add_icon"><span class="icon-add"></span></a></div><div class="right_section"><div class="right_top_section">';
                        }
        ...
        protected function getTinyHtmlTextArea( $param = array(), $param_value ) {
            ...
            if ( function_exists( 'wp_editor' ) ) {
                ...
                wp_editor( $default_content, 'spb_tinymce_' . $param['param_name'], array(
                        ...
                        'editor_height' => 350,
                        'tinymce' => array(
                            'fixed_toolbar_container' => '#edit-modal-header'
                        )
                    ) );
